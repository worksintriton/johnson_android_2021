package com.triton.johnson.utils;
...
public class ClockDrableTwo extends Drawable implements Animatable {

    private final static int ANIMATION_DURATION = 500;

    private static final @ColorRes
    int FACE_COLOR = android.R.color.white;
    private static final @ColorRes int RIM_COLOR = R.color.gray_btn_bg_color;
    ...
    public ClockDrableTwo(Resources resources) {
    ...
    protected void onBoundsChange(Rect bounds) {
        ...
        hourHandPath.addRect(bounds.centerX(), top, bounds.centerX(), top - hourHandLength, Path.Direction.CCW);
        hourHandPath.close();

        minuteHandPath.reset();
        minuteHandPath.moveTo(bounds.centerX(), bounds.centerY());
        minuteHandPath.addRect(bounds.centerX(), top, bounds.centerX(), top - minuteHandLength, Path.Direction.CCW);
